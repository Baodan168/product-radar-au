#!/usr/bin/env python3
"""
BrowserAct Fetcher — Wrapper for BrowserAct CLI to search Amazon AU.
...
def search_amazon(keyword, max_products=5, category="Search", retry=0):
    """Search Amazon AU via BrowserAct, return products in radar-compatible format.
    ...
    try:
        ...
        for i, item in enumerate(raw_items[:max_products]):
            ...
            products.append({
                ...
                "amazon_url": f"https://www.amazon.com.au/dp/{item['asin']}",
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        for p in products:
            print(f"  A${p['price']:.2f} | {p['name'][:60]} | {p['reviews']} reviews")
