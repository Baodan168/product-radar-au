...
同时修 audit P1 的 Worker 域名边界问题：endsWith('amazon.com.au')
会放行 evilamazon.com.au，必须按点号边界匹配。
"""
...
def host_allowed(hostname: str) -> bool:
    ...
    用点号边界而不是 endsWith —— 后者会把 evilamazon.com.au 判成
    amazon.com.au 的子域（audit P1）。
    """
    if not hostname:
        return False
