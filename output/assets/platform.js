...
function renderDiscovery() {
  ...
  list.innerHTML = insights.map((ins, idx) => {
    ...
    // Google Trends
    const gtUrl = `https://trends.google.com/trends/explore?geo=AU&q=${encodeURIComponent(amzKw)}`;
            ...
            if (matched.length > 0) {
              radarHtml = `<div class="radar-match"><div class="label">📡 雷达验证（已找到${matched.length}个产品）</div>` +
                matched.map(p => {
                  ...
                  return `<div class="radar-product">✅ ${esc((p.name||'').substring(0,45))} — A$${(p.price||0).toFixed(2)} | ${reviews}评论 ${ocean} | 利润率${((p.profit_margin||0)*100).toFixed(0)}%</div>`;
                }).join('') + `</div>`;
            }
          }
        }
...
function renderRadar() {
  ...
  grid.innerHTML = filtered.map(p => {
    ...
    return `<div class="pc" data-status="${escAttr(st)}" data-asin="${escAttr(p.asin||'')}">
      ...
      <div class="pc-meta"><span>💵 A\$${(p.price||0).toFixed(2)}</span><span>⭐ ${p.rating||'-'} (${p.reviews||0})</span>${p.first_seen?`<span>发现: ${p.first_seen}</span>`:''}</div>
