...
def filter_products(products, config):
    ...
    for p in products:
        ...
        if price < config["price_range"]["min"] or price > config["price_range"]["max"]:
            rejected.append({"name": name[:60], "reason": f"A${price} 不在区间", "asin": p.get("asin")}); continue
...
def main():
    ...
    print("[1/8] Amazon AU (New+BSR+Wished+Gifts)...", file=sys.stderr)
    ...
    #     重建版用 curl_cffi（sources.amazon_au._curl_fetch），Scrapling 被 Amazon 空响应拦截。
    try:
    ...
    for p in passed[:10]:
        ...
        print(f"    [{p['score']:3d}] A${p['price']:.2f} {p['profit_margin']*100:.0f}% | {src_tags} | {p['name'][:45]}", file=sys.stderr)
