...
def build_html(current, previous, all_valid, scan_date):
    ...
    for i, p in enumerate(current, 1):
        ...
        asin_link = f'<a href="https://www.amazon.com.au/dp/{asin}" target="_blank" class="product-name">{name}</a>' if asin else f'<span class="product-name">{name}</span>'
...
<div class="oa-hero">
  <div class="oa-hero-text">
    ...
    <p class="oa-page-desc"><strong>{total_scanned}</strong> 个产品扫描 · <strong style="color:#FF9500">{passed}</strong> 个需补货 · 数据来源: 领星ERP + Amazon AU</p>
  </div>
</div>
