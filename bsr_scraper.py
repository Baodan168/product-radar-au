...
Scrapes Amazon AU product pages for BSR, reviews, rating, image.
Usage:
...
def estimate_daily_sales(bsr: int, category: str = "general") -> int:
    """Estimate daily sales from BSR rank (Amazon AU)."""
    MULTIPLIERS = {
...
async def scrape_product(page, asin: str) -> dict:
    """Scrape a single Amazon AU product page for BSR and details."""
...
async def scrape_batch(asins: list[str], concurrency: int = 3) -> list[dict]:
    ...
    async with async_playwright() as p:
        ...
        await context.add_cookies([
            {"name": "sp-cc", "value": "1", "domain": ".amazon.com.au", "path": "/"},
