#!/usr/bin/env python3
"""
Seasonal Sourcing Trigger — generates keyword reports for upcoming AU events.
...
def _search_amazon_au(query: str, max_results: int = 5) -> str:
    """Search Amazon AU via AnySearch CLI. Returns raw text results."""
    cmd = [
        sys.executable, str(ANYSEARCH_CLI), "search",
        f"Amazon AU {query} trending best sellers",
        "--domain", "ecommerce",
        "--zone", "intl",
...
def run(output_json: bool = False) -> str:
    ...
    for event in actionable:
        ...
        if new_kws:
            ...
            for kw in new_kws:
                report_lines.append(f"  - {kw} (Amazon AU trend)")
        else:
