...
def test_legacy_shim_holds_only_variable_aliases(css):
    ...
    non_root = [x.strip() for x in selectors if x.strip() not in (':root', '.to-top', '.to-top:hover')]
    assert not non_root, f'shim 区混进了组件样式：{non_root}'  # .to-top 已知豁免  # .to-top 是 portal.js 动态生成的 UI，按历史约定从 shim 里放行
