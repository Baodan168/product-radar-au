...
@pytest.mark.parametrize('bad', [
    ...
    'http://www.amazon.com.au/dp/X',          # 非 https
    'https://evilamazon.com.au/dp/X',         # 后缀匹配能骗过，点号边界不能
    'https://amazon.com.au.evil.com/x',
    'https://user:pass@amazon.com.au/x',      # 内嵌凭据
    'https://amazon.com.au:8080/x',           # 非默认端口
    'https://evil.com/x',
    '',
    None,
])
...
@pytest.mark.parametrize('good', [
    'https://www.amazon.com.au/dp/B0DKBV52GF',
    'https://amazon.com.au/s?k=test',
...
def test_subdomain_boundary_not_suffix_match():
    """audit P1 的核心：endsWith 会把 evilamazon.com.au 当成 amazon.com.au。"""
    assert urls.host_allowed('www.amazon.com.au')
    assert not urls.host_allowed('evilamazon.com.au')
    assert not urls.host_allowed('notamazon.com.au')
...
def test_festival_sources_include_repo_local_fallback():
    ...
    assert any('au_festivals_data.js' in p for p in paths), '缺仓库内回退源'
    assert len(paths) >= 2
...
def test_festival_data_file_has_no_unflagged_compliance_violations():
    ...
    这里直接读仓库内的 data/au_festivals_data.js（不走
    festival_engine.load_festivals() 的多源回退——AU版已无UK源，
    测试要认仓库里提交的这份），跑一遍真实的 is_forbidden()，
    确保没有漏网的。
    ...
    content = (FESTIVAL_BASE / 'data' / 'au_festivals_data.js').read_text(encoding='utf-8')
    js_array = _extract_js_array(content, 'const FESTIVALS = ')
    assert js_array, 'data/au_festivals_data.js 里没找到 FESTIVALS 数组'
    data = _parse_js_array(js_array)
    assert data, 'data/au_festivals_data.js 解析失败或为空'
