...
def _run_anysearch(query, domain="general", max_results=8, freshness="week"):
    # ⚠️ 2026-08-26 发现：anysearch CLI 没有 "general" 域名（合法值见
    # anysearch_cli.py AVAILABLE_DOMAINS），"general" 会让 CLI 秒退 usage error，
    # 之前 11/24 个查询一直静默失败。这里映射到语义最近的合法域名。
    if domain == "general":
        domain = "home"  # 泛化查询 → home 域（覆盖家居/日用场景）
    try:
...
def fetch_trend_signals():
    ...
    query_groups = {
        "tiktok": [
            (f"TikTok trending products Australia {season} {year}", "ecommerce"),
        ...
        "temu": [
            ...
            ("Temu trending products Australia small items accessories gadgets", "ecommerce"),
            ("Temu viral products Australia what to sell on Amazon AU", "ecommerce"),
        ],
        "etsy": [
            (f"Etsy Australia trending products {season} {year} best sellers", "ecommerce"),
            ("Etsy Australia trending handmade accessories home decor", "ecommerce"),
            ("Etsy trending now Australia popular gift ideas", "ecommerce"),
        ],
        "youtube": [
            ("site:youtube.com Amazon AU haul best finds under $20 2026", "general"),
            ("youtube Amazon AU best sellers review small items haul", "general"),
            (f"youtube Amazon AU {season} must haves trending products", "general"),
        ],
        "google_trends": [
            (f"Australia trending products {season} {year} popular buying", "general"),
            (f"Amazon AU new releases trending {month} {year}", "general"),
            (f"{season} products Australia home garden outdoor trending", "general"),
        ],
        "reddit": [
            ("site:reddit.com Australia Amazon best cheap finds under $20", "general"),
            ("site:reddit.com australia small purchases improved life", "general"),
            ("site:reddit.com AusFrugal best value Amazon purchases", "general"),
        ],
        "market_intel": [
            (f"Amazon AU best sellers small items {year} trending accessories", "ecommerce"),
            (f"Australia consumer trends {year} popular products home garden", "general"),
            (f"cross border ecommerce Australia trending {season} {year}", "general"),
        ],
    }
