#!/usr/bin/env python3
"""
Keyword Matcher — Auto-match discovery keywords to Amazon AU products.

Uses AnySearch to find Amazon AU products matching a keyword,
...
def _scraperapi_search(keyword, max_results=5, api_key=None):
    """Search Amazon AU via ScraperAPI for structured results."""
    import urllib.request, urllib.parse
    search_url = f"https://www.amazon.com.au/s?k={urllib.parse.quote_plus(keyword)}&rh=p_36:559-1000"
...
def _parse_amazon_html(html, keyword, max_results=5):
    """Parse Amazon AU search results HTML into product list."""
...
def match_keyword_to_products(keyword, max_results=5):
    """Match a discovery keyword to Amazon AU products.
    ...
    if len(products) < max_results:
        ...
        query = f"Amazon AU {keyword} price reviews rating buy"
    ...
    print(f"  [{keyword}] Market: avg_reviews={summary['avg_reviews']} "
          f"median_price=A${summary['median_price']} "
...
if __name__ == "__main__":
    ...
    for i, p in enumerate(results, 1):
        print(f"  {i}. {p['name'][:80]}")
        print(f"     A${p['price']} | {p['reviews']} reviews | {p['rating']}★ | {p['asin']}")
    if not results:
