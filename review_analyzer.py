...
Amazon AU, then summarizes themes and suggests differentiation opportunities.
...
def analyze_negative_reviews(keyword: str, max_products: int = 5) -> dict:
    """
    Search for common negative reviews/feedback for a keyword on Amazon AU.
    ...
    if not all_text.strip():
        return {
            ...
            "negative_themes": [],
            "differentiation_opportunities": ["数据不足，无法分析差评。建议手动搜索 Amazon AU 查看 top 5 listing 的 1-3 星评论。"],
            "raw_snippet": ""
        }
...
def get_differentiation_tips(keyword: str, max_products: int = 5) -> str:
    ...
    if not themes:
        return f"[{keyword}] 差评数据不足，建议手动查看 Amazon AU 1-3 星评论。"
