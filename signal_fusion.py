...
Layer 2 (Supply-Demand Gap): Amazon AU top-20 analysis — avg reviews, ratings, price
...
def _score_trend_layer(keyword):
    ...
    queries = [
        (f"{keyword} trending Amazon AU {year}", "ecommerce"),
        (f"TikTok AU {keyword} viral trending", "ecommerce"),
        (f"Google Trends AU {keyword} {season} {year} rising search volume", "general"),
    ]
...
def _score_gap_layer(keyword):
    """Layer 2: Analyze Amazon AU top results for supply-demand gap.

    Uses keyword_matcher.get_market_summary() for real Amazon AU data via ScraperAPI.
    ...
    try:
        ...
        confidence = "low"
        query = f"Amazon AU {keyword} price reviews buy"
    ...
    # Updated thresholds based on realistic Amazon AU data
    if avg_reviews < 30:
...
if __name__ == "__main__":
    ...
    if output_json:
        ...
        if pw["median_competitor_price"] > 0:
            print(f"\n  Profit Window:")
            print(f"    Median competitor price: A${pw['median_competitor_price']}")
            print(f"    In target range (A${PRICE_MIN}-A${PRICE_MAX}): {pw['in_target_range']}")
            print(f"    Max 1688 cost for 20%+ margin: ¥{pw['max_1688_cost_cny']}")
            print(f"    Max sourcing in GBP: A${pw['max_sourcing_gbp']}")
